...
const RapportConsommationClientPage = () => {
  const [mois, setMois] = useState(dayjs().format("YYYY-MM"));
  const [donnees, setDonnees] = useState([]);

  const fetchRapport = async () => {
    try {
      const res = await ApiService.rapports.getConsommation(mois);
      setDonnees(res.data);
    } catch (err) {
      console.error("Erreur chargement rapport consommation :", err);
    }
  };
  ...
  const handleExport = async () => {
    try {
      const res = await ApiService.rapports.exportConsommationExcel(mois);
  ...
  const handleExportPDF = async () => {
    try {
      const res = await ApiService.rapports.exportConsommationPDF(mois);
  ...
  return (
    <div className="p-6 text-gray-800">
      <h2 className="text-2xl font-bold mb-6 text-blue-700">Rapport de Consommation Mensuelle</h2>
                ...
                <tbody>
                  {donnees.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="p-2 border">{item.consommation_kwh.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export default RapportConsommationClientPage;
