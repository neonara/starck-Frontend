...
import ProductionChart from "../components/carte-géo.jsx/ProductionChart";
import AlarmesChart from "../components/carte-géo.jsx/AlarmeChart";
...
const InstallationMapInstallateur = () => {
  ...
  useEffect(() => {
    ApiService.getInstallationsGeoDataInstallateur()
      .then((res) => {
        console.log("Installations reçues :", res.data.results);
        setInstallations(Array.isArray(res.data.results) ? res.data.results : []);
      })
      .catch((err) => console.error("Erreur API Installations:", err));
  }, []);
        ...
        <>
          ...
          <div className="absolute top-6 left-6 bg-white shadow-md rounded-lg px-4 py-3 z-[1000] w-[260px]">
            ...
            <p className="text-xl font-bold text-green-600">
              {installations.find(inst => inst.id === selectedInstallationId)?.revenus_mensuels_dt || 0} DT
            </p>
          </div>
        </>
      )}
...
export default InstallationMapInstallateur;
